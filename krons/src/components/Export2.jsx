...
const Export2 = ({setAlert}) => {
    ...
    const handleSubmit  = async (e) => {
      ...
      if (startDate > endDate){
        ...
        try {
          const response = await axios.post('http://localhost:8000/api/exportToPdf', {startDate:startDate,endDate:endDate},
            {
              headers: {
      ...
      const Address = () => (
        <>
          <View style={styles.titleContainer}>
            <View style={styles.spaceBetween}>
              <View>
                <Text style={styles.invoice}>Raktár készlet leltárív</Text>
      ...
      const TableHead = () => (
        <View style={{ width: '100%', flexDirection: 'row', marginTop: 10 }}>
          <View style={styles.theader}><Text style={styles.text}>Termék:</Text></View>
          <View style={styles.theader}><Text style={styles.text}>Mértékegység:</Text></View>
          <View style={styles.theader}><Text style={styles.text}>Átlagár:</Text></View>
          <View style={styles.theader}><Text style={styles.text}>Mennyiség:</Text></View>
          <View style={styles.theader}><Text style={styles.text}>Érték:</Text></View>
          <View style={styles.theader}><Text style={styles.text}>Fellelt mennyiség:</Text></View>
          <View style={styles.theader}><Text style={styles.text}>Megjegyzés</Text></View>
        </View>
      );
                ...
                <Fragment key={id}>
                  {groupedData[id].map((item, index) => {
                    ...
                    <View style={{ width: '100%', flexDirection: 'row' }}>
                        ...
                        <View style={styles.tbody}>
                          <Text>{(subTotal/subVol).toFixed(2)}</Text>
                        </View>
                        <View style={styles.tbody}>
                          <Text>{(subVol).toFixed(0)}</Text>
                        </View>
                        <View style={styles.tbody}>
        ...
        <form className="w-full xl:w-11/12 mb-12 xl:mb-0 px-4 mx-auto mt-4" >
            ...
            <div className="rounded-t mb-0 px-4 py-3 border-0">
                ...
                <div className="relative w-full px-4 max-w-full flex-grow flex-1">
                    <h3 className="font-semibold text-base py-4 text-blueGray-700 uppercase">Jegyző könyv</h3>
